...
- **Windowed UI** — every app opens in a draggable, resizable, minimizable, maximizable window with Aero-style glass chrome
...
- **Settings** — accent colour (8 presets), clock format (12h/24h), desktop icon size, and wallpaper picker — all persisted via `localStorage`
- **Mobile-responsive** — windows go full-screen, icons reflow, taskbar simplifies; double-tap to open apps on touch devices
...
| **Contact** | Contact links (email, LinkedIn, GitHub, Discord) |
| **File Explorer** | Virtual filesystem browser |
| **Photos** | Image viewer with prev/next navigation |
| **Settings** | Wallpaper, accent colour, clock format, icon size |
...
css/
  os.css            # Desktop, wallpaper layer, icon sizing, global resets
  windows.css       # Window chrome, animations, mobile overrides
  taskbar.css       # Taskbar & tray
  startmenu.css     # Start menu
  apps.css          # Per-app content styles + Settings UI
js/
  utils.js          # Shared helpers (iconHTML, etc.)
  windowManager.js  # Window lifecycle (open, close, minimize, maximize, drag, resize, touch)
  fs.js             # Virtual file system (FS_TREE, FS.listFolder, FS.openNode, PHOTO_MANIFEST)
  taskbar.js        # Taskbar button management + shutdown animation
  main.js           # Boot sequence, Wallpaper engine, ThemeEngine, DOMContentLoaded init
  apps/
    ...
    proj_fractal.js
    resume.js
    settings.js     # Settings app (wallpaper, accent, clock, icon size)
    skills.js
    welcome.js
icons/              # .ico assets (Vista icon set + utilities)
photos/             # Images served into the virtual Photos folder
images/             # Shutdown GIF + project screenshots
```
...
- User preferences stored in **`localStorage`** (wallpaper, accent, clock format, icon size)
...
APP_REGISTRY.push({
  ...
  title: 'My App',
  iconPath: 'icons/...',
  singleton: true,       // false = allow multiple windows
  showOnDesktop: true,   // false = start menu / programmatic only
...
Drop image files into the `photos/` folder, then register them in `js/fs.js`:
...
They'll appear in `C:\Photos` in the File Explorer and in the Settings wallpaper picker automatically.
